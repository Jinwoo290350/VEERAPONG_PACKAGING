  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-amber-500 text-forest-950 shadow-lg shadow-forest-950/20 transition hover:scale-105 hover:bg-amber-400"
      >
        {open ? (
      ...
      {open && (
        <div className="fixed bottom-24 right-5 z-50 flex h-[min(560px,calc(100dvh-8rem))] w-[min(400px,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
          <div className="bg-forest-950 px-5 py-4 text-white">
          ...
          <div className="border-t border-slate-100 p-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                className="max-h-28 flex-1 resize-none rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm outline-none transition focus:border-amber-500"
              />
              <button
                type="button"
                onClick={send}
                disabled={busy || !input.trim()}
                className="rounded-xl bg-forest-900 px-4 py-2.5 text-sm font-bold text-white transition enabled:hover:bg-forest-800 disabled:opacity-40"
              >
                {t("send")}
              </button>
            </div>
    ...
    <div className={`flex ${role === "user" ? "justify-end" : "justify-start"}`}>
      <div
        className={`chat-bubble max-w-[85%] whitespace-pre-wrap rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
          role === "user"
            ? "rounded-br-sm bg-forest-900 text-white"
